package com.zhongmingyuan.fleamarket.web.local;
...
import com.zhongmingyuan.fleamarket.enums.LocalAuthStateEnum;
import com.zhongmingyuan.fleamarket.exceptions.LocalAuthOperationException;
import com.zhongmingyuan.fleamarket.service.LocalAuthService;
import com.zhongmingyuan.fleamarket.util.CodeUtil;
import com.zhongmingyuan.fleamarket.util.HttpServletRequestUtil;
...
public class LocalAuthController {
    ...
    private Map<String, Object> bindLocalAuth(HttpServletRequest request) {
        ...
        //从session中获取当前用户的信息(用户一旦通过微信登录以后，便能获取用户的信息)
    ...
    private Map<String, Object> changeLocalPwd(HttpServletRequest request) {
        ...
        //从session中获取当前用户信息（用户一旦通过微信登录之后，便能获得用户信息）
        PersonInfo user = (PersonInfo) request.getSession().getAttribute("user");
        //非空判断，要求账号新旧密码以及当前用户的session非空，且新旧密码不相同
        if (userName != null && password != null && newPassword != null && user != null && user.getUserId() != null
                && !password.equals(newPassword)) {
            try {
                //查看原先账号，看看与输入账号是否一致，不一致则认为是非法操作
